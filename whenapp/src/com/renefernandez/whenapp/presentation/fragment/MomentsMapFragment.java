...
public class MomentsMapFragment extends SupportMapFragment {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		ArrayList<Moment> list = new ArrayList<Moment>(Arrays.asList(TestData.getTestMoments()));
		
		//map = this.getMap();
		
        markers = new ArrayList<MarkerOptions>(list.size());
        for (Moment moment : list) {
        	markers.add(new MarkerOptions().position(moment.getCoordinates())
            ...
            for (MarkerOptions marker : markers) {
                ...
                if(currentMarker==markers.size()){
        			...
        			map.moveCamera(CameraUpdateFactory
        					.newLatLngZoom(marker.getPosition(), 12));
